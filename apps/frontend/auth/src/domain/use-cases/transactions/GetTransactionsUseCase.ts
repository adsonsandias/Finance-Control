...
export class GetTransactionsUseCase {
  ...
  async execute(filters?: ITransactionFilters): Promise<IPaginatedResponse<ITransaction>> {
    // Validações e valores padrão
    const validatedFilters: ITransactionFilters = {
      page: filters?.page && filters.page > 0 ? filters.page : 1,
      limit: filters?.limit && filters.limit > 0 && filters.limit <= 100 ? filters.limit : 10,
      type: filters?.type,
