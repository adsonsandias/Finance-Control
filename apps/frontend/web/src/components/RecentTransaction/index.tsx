...
import { RecentTransactionItemStyles, MonthTransactionItemStyles } from './styles'
  ...
  return (
    <>
      <RecentTransactionItemStyles>
        ...
        {transactions &&
          transactions.map(({ id, title, type, amount, category, createdAt }: ITransaction) => (
            <TransactionItem
              key={id}
              title={title}
              type={type}
              ...
              date={createdAt ? new Intl.DateTimeFormat('pt-BR').format(new Date(createdAt)) : ''}
            />
          ))}
      </RecentTransactionItemStyles>
